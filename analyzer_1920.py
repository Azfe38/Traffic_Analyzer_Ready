...
Bu modül default olarak 25 fps ve 1920x1080 çözünürlük değerindedir.
...
Bu koordinatlar 1920X1080 Çözünürlüklü görüntü içindir.\n
...
.. code-block:: python
    ...
    # bu koordinatlar 1920X1080 Çözünürlüklü görüntü içindir.
    line = [(869, 720), (1290, 712)]  # çizgi 1 sarı , coordinates from (x1,y1) to (x2,y2)
    line2 = [(588, 658), (589, 443)]  # çizgi 2 kırmızı
    line3 = [(260, 469), (527, 417)]  # çizgi 3 mavi
    line4 = [(1160, 390), (1435, 338)]  # çizgi 4 turkuaz
...
.. code-block:: python
    ...
    if __name__ == '__main__':
        ...
        ap.add_argument("-o", "--output", required=True,  # output edilecek videonun adresi ve adı.uzantısı, uzantısı
...
# python analyzer_1920.py --input girdi/DJI_0096.MP4 --output cikti/DJI_0096_analiz.avi --yolo yolo-coco
...
if __name__ == '__main__':
    ...
    ap.add_argument("-y", "--yolo", required=True,  # yolonun ağırlık, etiket ve cfg dosyalarının adresi
                    help="base path to YOLO directory")
    ap.add_argument("-c", "--confidence", type=float, default=0.30,  # tanımadaki eminlik değeri default'u 0.5'ti #0.47
                    help="minimum probability to filter weak detections")
    ap.add_argument("-t", "--threshold", type=float, default=0.3,
                    # non-maxima suppression için eşik değeri default'u 0.5'ti
                    help="threshold when applyong non-maxima suppression")
    ...
    while True:
        ...
        if len(boxes) > 0:
            ...
            for box in boxes:
                ...
                if indexIDs[i] in previous:
                    ...
                    p0 = (int(x + (w - x) / 2), int(y + (h - y) / 2))
                    p1 = (int(x2 + (w2 - x2) / 2), int(y2 + (h2 - y2) / 2))
                    cv2.line(frame, p0, p1, color, 15)  # default 3 tespit edilen nesnenin üzerine nokta koyulması
                    ...
                    if intersect(p0, p1, line4[0], line4[1]):
                        if (classIDs[i] == 1 or classIDs[i] == 2 or classIDs[i] == 3 or
                                classIDs[i] == 5 or classIDs[
                                    i] == 7):  # sadece bicycle,car, motorbike,bus,truck sayılır
                            counter4 += 1
                            if classIDs[i] == 2:  # classIDs[i] = otomobil ise carCounter4 artar
                                carCounter4 += 1
                            elif classIDs[i] == 7:  # classIDs[i] = kamyon ise truckCounter4 artar
                            ...
                            if (idBul in countedByLine1):  # yol tayin için çizgiye değenin adını liste 1'de ara
                                ...
                                print("counter 5 = ", counter5,
                                      LABELS[classIDs[i]])  # yol 1 sayma bilgisini consola yaz
        ...
        # draw line  # çizgilerin ekranda gösterildiği yer
        cv2.line(frame, line[0], line[1], (0, 255, 255), 20)  # Blue Green Red, Çizgi Kalınlığı
        cv2.line(frame, line2[0], line2[1], (0, 0, 255), 20)
        cv2.line(frame, line3[0], line3[1], (255, 0, 0), 20)
        cv2.line(frame, line4[0], line4[1], (255, 255, 0), 20)

        # draw counter  #  sayıcıların ekranda gösterildiği yer
        cv2.putText(frame, str(counter), (1310, 730), cv2.FONT_HERSHEY_DUPLEX, 2.0, (0, 255, 255), 5)  # sarı
        cv2.putText(frame, str(counter2), (599, 423), cv2.FONT_HERSHEY_DUPLEX, 2.0, (0, 0, 255), 5)  # kırmızı
        cv2.putText(frame, str(counter3), (537, 370), cv2.FONT_HERSHEY_DUPLEX, 2.0, (255, 0, 0), 5)  # mavi
        cv2.putText(frame, str(counter4), (1425, 310), cv2.FONT_HERSHEY_DUPLEX, 2.0, (255, 255, 0), 5)  # turkuaz
        cv2.putText(frame, str(counter5), (1490, 220),
                    cv2.FONT_HERSHEY_DUPLEX, 2.0, (255, 255, 255), 5)  # beyaz (600,150) yön1
        cv2.putText(frame, str(counter6), (690, 300),
                    cv2.FONT_HERSHEY_DUPLEX, 2.0, (255, 255, 255), 5)  # beyaz (600,150) yön2
        ...
        if writer is None:
            ...
            # cv2.imshow("Kamera", frame)
            fourcc = cv2.VideoWriter_fourcc(*"MJPG")
            writer = cv2.VideoWriter(args["output"], fourcc, 25,  # default 30 fps idi, input video fps
                                     (frame.shape[1], frame.shape[0]), True)
    ...
    # sonuçların yazdırıldığı kısım
    toText = open("AnalizSonucu.txt", "a")  # sonuçları yeni satır olarak eklemek için append modunda txt dosyası açıyor
    toText.write("Analiz Tarihi = {0} | ".format(str(an)))
